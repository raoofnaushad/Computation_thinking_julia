...
author = (name = "Raoof Naushad", email = "raoofnaushad.7@gmail.com")
...
Created by: **_$(author.name)_** ($(author.email))
"""
...
md"""
## **1** - _Manipulating vectors (1D images)_
...
function create_bar()
	...
	vect = fill(0, tot)
	half = tot ÷ 2
	vect[half-(cent÷2):half+(cent÷2)] .= 1
	return vect
end
...
md"""
#### Exercise 2.1
👉 Write a function **`mean_colors`** that accepts an object called `image`. It should calculate the mean (average) amounts of red, green and blue in the image and return a tuple `(r, g, b)` of those mean
"""
...
# ╔═╡ b863580c-07ae-11eb-181a-6bc7648fd0a9
floor(0.267 * 10) ÷10
...
The image is unrecognisable with intensity 2.5
"""
...
# ╔═╡ d3713656-07b2-11eb-346e-91b2623fbe90
philip
...
# ╔═╡ d43b1182-07bf-11eb-021f-253cef3e5dd0
@bind l_box Slider(0:1:20, show_value=true)

# ╔═╡ d2ff6e56-07bf-11eb-3cc3-25629da8378c
begin
	new_v = blur_1D(v, l_box)
	colored_line(new_v)
end
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	l_i = (m_K-1) ÷ 2
	l_j = (n_K-1) ÷ 2
	for i in range(1, length=m_M)
		for j in range(1, length=n_M)
			avg = RGB(0)
			for a = -l_i:l_i
				for b = -l_j:l_j
				avg += K[m_K÷2+a+1, n_K÷2+b+1] * extend_mat(M, i+a, j+b)
				end
			end
		NEW_M[i, j] = avg
		end
...
# ╔═╡ c957abd0-0872-11eb-2905-0f60e87e154f
K_test = [
	0   0  0
	1/2 0  1/2
	0   0  0
...
# ╔═╡ 071cb7e6-0879-11eb-1b5d-338aa44a4a12
convolve_image(philip, K_test)

# ╔═╡ 1dd79d4c-0879-11eb-2108-b7935fa6c07a
another_K = [
	0 0 0
	0.5 0 0.5
	0 0 0 
...
# ╔═╡ 1aa65922-0879-11eb-35cf-19887e7b0b33
convolve_image(philip, another_K)
...
# ╔═╡ a99d49e2-087b-11eb-2d25-addf37cb588a
another_K

# ╔═╡ 8b927912-087a-11eb-2f77-277b9e0f791b
@bind kernel_size Slider(0:1:10, show_value=true)

# ╔═╡ 05d7d826-087f-11eb-229a-a72f196f16e6
begin
	nn = 3
	σ = 20
	temp = [exp(-(x^2+y^2)/2σ^2)/(2π*σ^2) for x=1:nn,y=1:nn]
	G = [temp[x,y]/sum(temp) for x=1:nn,y=1:nn]
	G
end

# ╔═╡ 1c8b4658-ee0c-11ea-2ede-9b9ed7d3125e
function gaussian_kernel(n)
    x = range(-n^2/2, n^2/2,length=10)
    G(x) = exp((-x ^ 2) / 2) / sqrt(2 * π)
    y = G.(x)
    return y ./ sum(y)
end
...
# ╔═╡ b3a038f8-087b-11eb-0489-05302852ffb8
...
function with_gaussian_blur(image)
    n = 3
    σ = 20
    temp = [exp(-(x^2+y^2)/2σ^2)/(2π*σ^2) for x=1:n,y=1:n]
    G = [temp[x,y]/sum(temp) for x=1:n,y=1:n]
    return convolve_image(image, G)
end
